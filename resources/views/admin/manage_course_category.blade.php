...
@section('content')
            ...
            <div>
                <form method="POST" action="{{ route('admin.course_category.add') }}">
                    ...
                    <div class="mt-5 flex gap-2">
                        <button type="button" data-modalid="modal-add-course-category" class="closeModalBtn text-white bg-rose-500 hover:bg-rose-600 shadow-sm rounded-sm hover:shadow-md w-full font-semibold cursor-pointer p-2">
                            Cancel
                        </button>
                        <button type="submit" class="text-white bg-violet-500 hover:bg-violet-600 shadow-sm rounded-sm hover:shadow-md w-full font-semibold cursor-pointer p-2">
                            <i class="fa-solid fa-floppy-disk text-lg"></i> save
                        </button>
                    </div>
                </form>
            </div>
    ...
    <!--------modal edit course category----------->

    <div id="modal-edit-course-category-{{ $c->id }}" class="fixed inset-0 top-0 flex items-center justify-center hidden bg-white bg-opacity-50 z-10 p-5">
        <div class="rounded shadow-lg p-5 w-full md:w-md bg-white flex flex-col gap-5 overflow-y-auto max-h-[90vh]">
            ...
            <div class="">
                <form method="POST" action="{{ route('admin.course_category.update', ['id' => $c->id]) }}" enctype="multipart/form-data">
                    @csrf
                    <x-input label="title" name="title" type="text" value=" {{ $c->title }}"/>
                    <x-input label="description" name="description" type="text" value=" {{ $c->description }}"/>

                    <div class="mt-2 flex gap-2">
                        <button type="button" data-modalid="modal-edit-course-category-{{ $c->id }}" class="closeModalBtn
                        bg-rose-500 hover:bg-rose-600 text-white shadow-sm hover:shadow-md rounded-sm cursor-pointer p-1 w-full font-semibold">Cancel</button>
                        <button type="submit" class="bg-violet-600 hover:bg-violet-700 text-white shadow-sm hover:shadow-md rounded-sm cursor-pointer p-1 w-full font-semibold">Save</button>
                    </div>
                </form>
            </div>
        </div>
    </div>
    <!-----------eof modal edit course category---->
    @endforeach
            ...
            <div class="flex flex-col bg-white rounded-sm shadow-sm w-full p-5 gap-2 flex-wrap">
                ...
                <div class="flex flex-col gap-2">
                    <div class="flex justify-end mb-2">
                        <button type="button"  data-modalid="modal-add-course-category" class="openModalBtn bg-indigo-600 text-white font-semibold aspect-square w-8 rounded-full shadow-sm cursor-pointer shadow-sm openModalBtn hover:bg-indigo-700 hover:shadow-md">
                                ...
                                @foreach($course_categories as $c)
                                    <tr class="">
                                        ...
                                        <td class="border border-gray-300 py-2 px-2 flex gap-2 justify-center flex-wrap">
                                            <a class="rounded-sm bg-rose-500 hover:bg-rose-600 text-white hover:shadow-md p-1 font-semibold cursor-pointer px-2" href="{{ route('admin.course_category.delete', ['id' => $c->id]) }}">
                                                <i class="fa-solid fa-trash"></i> delete
                                            </a>
                                            <button type="button" class="openModalBtn rounded-sm bg-violet-500 hover:bg-violet-600 text-white hover:shadow-md p-1 font-semibold cursor-pointer px-2" data-modalid="modal-edit-course-category-{{ $c->id }}">
